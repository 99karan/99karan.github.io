...
function TimelineNode({
  ...
  useFrame((state, delta) => {
    ...
    if (light.current) light.current.intensity = 0.6 + a * 5.5;
    if (panel.current) {
    ...
    <group position={position}>
      ...
      <group ref={panel} position={labelOffset}>
        <HoloPanel
          ...
          width={2.05}
          height={0.72}
          position={[portrait ? 0.9 : 1.0, 0, 0]}
          parallax={0.05}
          float={0.02}
          phase={index * 1.3}
          deps={[milestone.id]}
        />
        <mesh position={[portrait ? 0.06 : 0.06, 0, 0]}>
          <planeGeometry args={[portrait ? 0.12 : 0.12, 0.5]} />
          <meshBasicMaterial color="#66e0c0" transparent opacity={0.25} toneMapped={false} />
        </mesh>
      </group>
    </group>
  );
...
export function ExperienceIsland() {
  ...
  return (
    <group scale={portrait ? 0.82 : 1}>
      <mesh geometry={rail}>
        <meshBasicMaterial color="#66e0c0" transparent opacity={0.42} toneMapped={false} />
      </mesh>
